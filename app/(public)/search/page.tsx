...
async function SearchContent({ searchParams }: SearchPageProps) {
  ...
  const makes = await getMakesForFacets(parsedParams);
  ...
  return (
    <MarketplaceShell>
      <div className="space-y-6">
        {/* Header */}
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-ink-900">Search Cars</h1>
        ...
        <div className="grid gap-6 md:grid-cols-4">
          ...
          <aside className="hidden md:block">
            <FiltersSidebar initialParams={parsedParams} makes={makes} />
          </aside>
                          ...
                          <PaginationItem>
                            <PaginationPrevious
                              href={`/search?${new URLSearchParams({
                                ...params,
                                page: (parsedParams.page - 1).toString(),
...
export default async function SearchPage({ searchParams }: SearchPageProps) {
  return (
    <Suspense fallback={<MarketplaceShell><div>Loading...</div></MarketplaceShell>}>
