...
export default async function AdminClaimsPage({ searchParams }: PageProps) {
  ...
  if (q.trim()) {
    // Filter by business name (ilike on joined table not directly supported — filter after fetch)
  }

  const { data: rawClaims, count } = await query;
  let claims = (rawClaims ?? []) as Claim[];

  // Client-side filter for search (business name or claimant email)
  if (q.trim()) {
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <div className="flex items-center gap-3">
            <h1 className="text-2xl font-black text-ink-900">Claims</h1>
