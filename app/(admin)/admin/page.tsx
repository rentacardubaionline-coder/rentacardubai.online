...
function StatCard({
  ...
  const iconClass = accent
    ? colors[accent]
    : "bg-surface-muted text-ink-500 ring-surface-muted";
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between">
        <div
          className={`flex h-10 w-10 items-center justify-center rounded-lg ring-1 ${iconClass}`}
        >
...
export default async function AdminDashboard() {
  ...
  const [
    ...
    (db as any)
      .from("business_claims")
      .select(
        "id, status, created_at, business:business_id(name, city), claimant:claimant_user_id(email, full_name)",
      )
      .eq("status", "pending")
    ...
    <div className="space-y-8">
      ...
      {stats.pendingKyc > 0 && (
        ...
        >
          ...
          />
          <span>
            <strong>{stats.pendingKyc}</strong> KYC submission
            {stats.pendingKyc !== 1 ? "s" : ""} waiting for review
          </span>
      ...
      {stats.pendingClaims > 0 && (
        ...
        >
          ...
          />
          <span>
            <strong>{stats.pendingClaims}</strong> pending claim
            {stats.pendingClaims !== 1 ? "s" : ""} waiting for review
          </span>
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        ...
        <StatCard
          ...
          sub={`${stats.approvedListings} live · ${stats.draftListings} draft`}
          icon={Car}
          href="/admin/listings"
          accent="blue"
        />
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs text-ink-500">
                      {claim.claimant?.email ?? "Unknown claimant"} ·{" "}
                      {claim.business?.city}
                    </p>
                  </div>
